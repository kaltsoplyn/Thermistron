...
static esp_err_t refresh_cached_config_and_adc() {
    ...
    if (ret != ESP_OK || s_adc_handle == NULL) {
        ESP_LOGE(TAG, "Failed to get ADC unit handle: %s", esp_err_to_name(ret));
        return ret;
    }
    ESP_LOGI(TAG, "ADC unit handle obtained.");

    s_cached_sampling_interval_ms = config_comp_get_sampling_interval();
    ESP_LOGI(TAG, "Using sampling interval: %d ms", s_cached_sampling_interval_ms);
    ...
    if (s_cached_active_therm_count < 0 || s_cached_active_therm_count > MAX_THERMISTOR_COUNT) {
        ESP_LOGW(TAG, "Invalid thermistor count from config: %d.", s_cached_active_therm_count);
...
static esp_err_t _measure_temperature(ThermistorConfig_t *thermistor, float *out_temperature) {
    ...
    if (ret != ESP_OK) {
        *out_temperature = NAN; // Indicate failure with NAN
        return ret;
    }
    ...
    if (adc_value <= 0 || adc_value >= max_adc_val) {
        ...
        *out_temperature = NAN;
        return ESP_ERR_INVALID_STATE; // Or a custom error indicating out of measurable range
    }

    float Rth = (float)divider_resistor * adc_value / (max_adc_val - adc_value);
    ...
    // Steinhart-Hart coefficients (example for a common 10k NTC like NTC3950)
    const float A = 0.001129148f;
    const float B = 0.000234125f;
    const float C = 0.0000000876741f;

    float logRth = logf(Rth); // Use logf for float
    ...
    while (1) {
        ...
        for (int i = 0; i < MAX_THERMISTOR_COUNT; ++i) {
            ...
            if (xSemaphoreTake(s_temp_data_mutex, portMAX_DELAY) == pdTRUE) {
                ...
                // ESP_LOGD(TAG, "Thermistor %s (%s): ADC %d, Temp: %.2f C", s_cached_therm_configs[i].name, s_cached_therm_configs[i].adc_channel, adc_raw_reading, current_temp);
        }
